(function () {
    ...
    var CategoryThreeLoader = miniProject.service('CategoryThreeLoader', function ($http, $rootScope) {
        this.promise = null;
        function makeRequest() {
...
//Controller for the third game category
    var categoryThree = miniProject.controller("CategoryThreeCtrl", function ($scope, $timeout, $window, $http, CategoryThreeLoader) {
    //Scope variables
    $scope.question;
    $scope.answer = "";
    $scope.success = false;
    ...
    $scope.checkAnswer = function () {
        if ($scope.answer.toLowerCase() === $scope.question.ColorName.toLowerCase()) {
            $scope.success = true;
            $scope.failure = false;
            _Score+=2;
            $scope.score = _Score;
